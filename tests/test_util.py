...
two_binaries = st.integers(min_value=0, max_value=10).flatmap(
    lambda i: st.tuples(st.binary(min_size=i, max_size=i), st.binary(min_size=i, max_size=i))
)
...
class TestIntegerUtils(unittest.TestCase):
    ...
    def test_to_inverts_from(self, bl):
        ...
        self.assertEqual(
            int(I.to_binary_str(I.from_binary_str(bs)), 2),
            int(bs, 2)
        )
...
class TestBytesUtils(unittest.TestCase):
    def test_to_bstr(self):
        self.assertEqual(
            B.to_binary_str( (0).to_bytes(1, byteorder='big') ),
            '0'
        )
        self.assertEqual(
            B.to_binary_str( (1).to_bytes(1, byteorder='big') ),
            '1'
        )
        self.assertEqual(
            B.to_binary_str( (4).to_bytes(1, byteorder='big') ),
            '100'
        )
