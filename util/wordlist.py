...
class Wordlist:
    ...
    def __init__(self):
        ...
        self.MAX_WORDLEN = reduce(lambda a, s: max(a, len(s)), self.wordlist, 0)
        self.m_words_to_idx = {self.wordlist[i]: i for i in range(n)}
        self.unique_prefixes = {
            self.wordlist[i][: self.unique_prefix_length]: self.wordlist[i]
            for i in range(n)
        }
